...
namespace OSImGui
{
    ...
    void OSImGui_External::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        ...
        // Split DestWindowName by commas
        std::vector<std::string> windowNames;
        size_t start = 0;
        size_t end = 0;
        while ((end = DestWindowName.find(',', start)) != std::string::npos)
        {
            windowNames.push_back(DestWindowName.substr(start, end - start));
            start = end + 1;
        }
        windowNames.push_back(DestWindowName.substr(start));
        ...
        for (const auto& windowName : windowNames)
        {
            ...
            if (hWnd != NULL)
            {
                DestWindow.hWnd = hWnd;
                DestWindow.Name = windowName;
                DestWindow.ClassName = DestWindowClassName;
                break;
            }
        }
        if (hWnd == NULL)
            throw OSException("None of the DestWindow names exist");
