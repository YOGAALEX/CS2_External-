...
#pragma region 请求接口获取数据
	...
	Model::Result NetworkVerHelp::GetRequestResult(Model::BusinessType businessType, string data)
	{
		...
		int repLogOutNum = 0;
		int SleepReqTime = 1;
		if (Model::BusinessType::remoteCalculate== businessType)
		{
...
#pragma region 远程算法结果转化成对象
	//远程算法结果转化成对象
	Model::Out_getCalculate NetworkVerHelp::ConvertCalculate(string data)
	{
		//VMProtectBegin("ConvertCalculate");
		Model::Out_getCalculate result;
		...
		oJson.Get("requestflag", result.requestflag);
		oJson.Get("servertimestamp", result.servertimestamp);
		oJson.Get("calculateresult", result.calculateresult);
...
#pragma region 获取软件变量
	...
	Model::Result NetworkVerHelp::getRemoteVar(Model::In_getRemoteVarArgs dataArgs)
	{
		...
		oJson.Add("cardnumorusername", dataArgs.cardnumorusername);
		oJson.Add("token", dataArgs.token);
		oJson.Add("varname", dataArgs.varname);
		data = oJson.ToString();
...
#pragma region 获取远程算法
	//获取远程算法
	Model::Result NetworkVerHelp::getCalculate(Model::In_getCalculateArgs dataArgs)
	{
		//VMProtectBegin("getCalculate");
		Model::Result result;
		string data = "";
		if (dataArgs.token == "")
		{
		...
		oJson.Add("maccode", dataArgs.maccode);
		oJson.Add("timestamp", dataArgs.timestamp);
		oJson.Add("calculateid", dataArgs.calculateid);
		oJson.Add("token", dataArgs.token);
		oJson.Add("requestargs", dataArgs.requestargs);
		data = oJson.ToString();

		result = GetRequestResult(Model::BusinessType::remoteCalculate, data);
		return result;
		//VMProtectEnd();
	}
#pragma endregion
